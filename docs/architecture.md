...
- `services/voice-agent`: BullMQ worker that starts outbound calls and runs the conversation. It supports mock mode today and has isolated calling adapters for mock, Vobiz SIP through LiveKit, and Vobiz Voice API.
...
7. In Vobiz API mode the `VobizApiCallingProvider` starts the outbound call and Vobiz posts call status webhooks back to the API.
...
- Calling is behind `CallingProvider`, so the app can switch between `mock`, `vobiz_sip`, and `vobiz_api` without changing campaign or booking code.
